...
function App() {
  let [connected, setConnectionStatus] = useState(false)

  useEffect(() => {
    async function Initialize(){
      let request = await fetch( baseAPI_URL + '/', {
        'method': 'GET', 
        'headers': {
  ...
  return (
    <div className='h-full w-full'>
      ...
      <div className={`absolute h-full w-full top-0 left-0 backdrop-blur flex justify-center items-center bg-slate-700 bg-opacity-80 z-80 ${connected ? 'hidden' : ''}`}>
        ...
        <span className="ml-4 text-xl text-rose-200">Please wait while we connect to the Server (Server is in Sleep State)</span>
      </div>
    </div>
  )
}
